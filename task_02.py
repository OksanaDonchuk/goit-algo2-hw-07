...
def fibonacci_lru(n: int) -> int:
    """Обчислює n-е число Фібоначчі, використовуючи кешування LRU."""
    if n <= 1:
        return n
...
class SplayTree:
    ...
    def _splay(self, root: Optional[SplayTreeNode], key: int) -> Optional[SplayTreeNode]:
        """Переміщує вузол з ключем `key` до кореня (splay-операція)."""
    ...
    def insert(self, key: int, value: int) -> None:
        """Вставляє значення у Splay Tree, якщо його ще немає."""
